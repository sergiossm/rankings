...
enum TextFieldType { filled, outlined, underlined }
...
class _CustomTextFieldState extends State<CustomTextField> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: widget.size.height, child: _buildTextField(context)),
      ...
      style: context.textStyle.bodyLarge.copyWith(
        color:
            widget.enabled
                ...
                : context.color.onSurface.withValues(alpha: 0.38),
      ),
    ...
    return switch (widget.type) {
      TextFieldType.filled => InputDecoration(
        ...
        hintStyle: context.textStyle.bodyLarge.copyWith(
          color: context.color.onSurface.withValues(alpha: 0.38),
        ),
      ...
      TextFieldType.outlined => InputDecoration(
        ...
        disabledBorder: OutlineInputBorder(
          ...
          borderSide: BorderSide(
            color: context.color.outline.withValues(alpha: 0.38),
          ),
        ),
        counterText: '',
      ),
      TextFieldType.underlined => InputDecoration(
        ...
        border: UnderlineInputBorder(
          borderSide: BorderSide(
            color: context.color.outline.withValues(alpha: .45),
          ),
        ),
        ...
        focusedBorder: UnderlineInputBorder(
          borderSide: BorderSide(
            color: context.color.secondary.withValues(alpha: .45),
          ),
        ),
  ...
  Color _getFillColor(BuildContext context) {
    if (!widget.enabled) {
      return context.color.surfaceVariant.withValues(alpha: 0.38);
    }
    if (_hasError) {
      return context.color.error.withValues(alpha: 0.12);
    }
    if (_isFocused) {
      return context.color.surfaceVariant.withValues(alpha: 0.16);
    }
    return context.color.surfaceVariant.withValues(alpha: 0.08);
  }

  Color _getTextColor(BuildContext context) {
    if (!widget.enabled) {
      return context.color.onSurface.withValues(alpha: 0.38);
    }
    if (_hasError) {
